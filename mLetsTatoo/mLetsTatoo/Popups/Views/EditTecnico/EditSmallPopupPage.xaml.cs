...
namespace mLetsTatoo.Popups.Views
{
	...
	public partial class EditSmallPopupPage : PopupPage
	{
        #region Attributes
        private int time;
        ...
        public async void GetImage(object sender, EventArgs e)
        {
            ...
            if (source == Languages.Cancel)
            {
                ...
                {
                    MainViewModel.GetInstance().EditFeaturesPopup.FSE.Imagen_Ejemplo = MainViewModel.GetInstance().EditFeaturesPopup.FSE.Imagen_Ejemplo;
                }
                else if (imageSender == this.SMImage)
                {
                    MainViewModel.GetInstance().EditFeaturesPopup.FSM.Imagen_Ejemplo = MainViewModel.GetInstance().EditFeaturesPopup.FSM.Imagen_Ejemplo;
                }
                else if (imageSender == this.SHImage)
                {
                    MainViewModel.GetInstance().EditFeaturesPopup.FSH.Imagen_Ejemplo = MainViewModel.GetInstance().EditFeaturesPopup.FSH.Imagen_Ejemplo;
                }
                return;
            }
            ...
            if (imageSender == this.SEImage)
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSE.Imagen_Ejemplo = FileHelper.ReadFully(this.file.GetStream());
            }
            else if (imageSender == this.SMImage)
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSM.Imagen_Ejemplo = FileHelper.ReadFully(this.file.GetStream());
            }
            else if (imageSender == this.SHImage)
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSH.Imagen_Ejemplo = FileHelper.ReadFully(this.file.GetStream());
            }
        }
        public async void GetTime(object sender, EventArgs e)
        {
            ...
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSE.Tiempo  = this.time;
            }
            else if (timeSender == this.SMTime)
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSM.Tiempo = this.time;
            }
            else if (timeSender == this.SHTime)
            {
                MainViewModel.GetInstance().EditFeaturesPopup.FSH.Tiempo = this.time;
            }
        }
